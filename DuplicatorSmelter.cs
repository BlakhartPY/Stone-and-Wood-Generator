...
public class StoneWoodGenerator : MonoBehaviour, IHasHoverMenuExtended
{
    ...
    private void DropAllItems()
    {
        ...
        if (this.m_fuelItem != null)
        {
            ...
            {
                Vector3 vector = base.transform.position + Vector3.up + global::UnityEngine.Random.insideUnitSphere * 0.3f;
                Quaternion quaternion = Quaternion.Euler(0f, (float)global::UnityEngine.Random.Range(0, 360), 0f);
                ItemDrop.OnCreateNew(global::UnityEngine.Object.Instantiate<GameObject>(this.m_fuelItem.gameObject, vector, quaternion));
            }
        }
        while (this.GetQueueSize() > 0)
        {
            ...
            {
                Vector3 vector2 = base.transform.position + Vector3.up + global::UnityEngine.Random.insideUnitSphere * 0.3f;
                Quaternion quaternion2 = Quaternion.Euler(0f, (float)global::UnityEngine.Random.Range(0, 360), 0f);
                ItemDrop.OnCreateNew(global::UnityEngine.Object.Instantiate<GameObject>(itemConversion.m_from.gameObject, vector2, quaternion2));
            }
        }
    }
    ...
    private void Spawn(string ore, int stack)
    {
        ...
        if (itemConversion != null && itemConversion.m_to != null)
        {
            ...
            ItemDrop component = global::UnityEngine.Object.Instantiate<GameObject>(itemConversion.m_to.gameObject, this.m_outputPoint.position, this.m_outputPoint.rotation).GetComponent<ItemDrop>();
    ...
    public bool TryGetItems(Player player, Switch switchRef, out List<string> items)
    {
        ...
        {
            items = this.m_conversion.Select((Smelter.ItemConversion conversion) => conversion.m_from.m_itemData.m_shared.m_name).ToList<string>();
        }
    ...
    public bool CanUseItems(Player player, Switch switchRef, bool sendErrorMessage = true)
    {
        ...
        if (switchRef == this.m_addOreSwitch)
        {
            ...
            if (player.GetInventory().CountItemsByName(this.m_conversion.Select((Smelter.ItemConversion conversion) => conversion.m_from.m_itemData.m_shared.m_name).ToArray<string>(), -1, true, false) > 0)
            {
                return true;
            }
            if (sendErrorMessage)
